...
- [3. UART Initialization](#3-uart-initialization)
- [4. Sending Data — TX Task](#4-sending-data--tx-task)
- [5. Receiving Data — RX Task](#5-receiving-data--rx-task)
...
**How it works:**
1. `uart_config_t` defines the communication parameters (baud rate, data bits, parity, stop bits, flow control) — all pulled from Kconfig instead of hardcoded values.
2. `uart_driver_install()` allocates the internal RX/TX ring buffers used by the driver.
3. `uart_param_config()` applies the configuration to `UART_NUM_1`.
4. `uart_set_pin()` maps the TX/RX signals to the physical GPIO pins defined in Kconfig.
...
**How it works:**
1. `uart_read_bytes()` blocks for up to 1000 ms waiting for incoming data, returning the number of bytes actually read.
2. If data was received, a null terminator is appended so it can be logged as a C string.
3. `ESP_LOG_BUFFER_HEXDUMP()` additionally prints the raw bytes in hex — useful for verifying non-printable characters (like `\r\n`) were received correctly.
